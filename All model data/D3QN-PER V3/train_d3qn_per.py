...
EPS_START        = 1.0
EPS_END          = 0.10
EPS_DECAY_STEPS  = 500_000   # ~50% of total steps for 1000 episodes
...
PER_BETA_STEPS   = 500_000   # anneal beta over same horizon as epsilon
...
class PrioritizedReplayBuffer:
    ...
    def sample(self, batch_size: int, beta: float):
        ...
        # Guard against division by zero when buffer is filling
        if min_prob <= 0:
            min_prob = 1e-8
        ...
        for i in range(batch_size):
            ...
            if transition is None:
                # Buffer not full yet — resample uniformly
...
def train(args):
    ...
    print(f"\nTraining D3QN-PER | episodes={args.episodes} | {wall_str} | difficulty=0")
    print("=" * 70)

    for ep in range(1, args.episodes + 1):
        raw_obs = env.reset(seed=ep)
        ...
        while not done:
            ...
            shaped   = reward + (-300 if wall_hit else 0)   # extra -300 internally
        ...
        if ep % LOG_EVERY == 0:
            ...
            if mean_r > best_mean:
                ...
                print(f"  ✓ New best (mean_r={best_mean:.1f}) → {SAVE_PATH}")
