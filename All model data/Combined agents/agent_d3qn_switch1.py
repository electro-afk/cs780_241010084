"""
D3QN Agent — Model Switching on First Wall Encounter
=====================================================
Starts every episode with the no-walls model.
Switches to the walls model the moment the first wall stuck event is detected.
...
Evaluate locally:
  python evaluate.py --agent_file agent_d3qn_switch1.py --difficulty 0 --wall_obstacles
"""
...
# Switch threshold — first wall event
WALL_SWITCH_THRESHOLD = 1
...
def policy(obs: np.ndarray, rng: np.random.Generator) -> str:
    ...
    Switches from no-walls model to walls model on the first wall stuck event.
    Switch resets at every episode boundary.
    """
    if _is_new_episode(obs):
        ...
        # ── Model switch check ────────────────────────────────────────────────
        # Switch on first wall hit — registry.wall_hit_count increments
        # inside register_stuck whenever is_wall=True, regardless of equation status
        if (not _STATE["wall_switched"] and
            ...
            print(f"[agent_switch1] Wall detected at step {_STATE['step']} "
                  f"— switching to walls model")
